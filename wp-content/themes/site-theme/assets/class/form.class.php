...
class send_form_mail extends base {
	...
	private $reload_message = '既にメッセージは送信済みです';
...
class formset extends base {
	...
	private $def_confirm_error_mesasge = '※ こちらは必須項目となっております';
...
//$this->debug_repotr .=  '$_SESSION'.'：'.nl2br(var_export($_SESSION,true)).'<br>';
	...
	public function submit ( $class_send = 'bc_blue', $class_back = 'bc_white', $data_theme_send = 'b', $data_theme_back = 'a' ) {
		...
		$tag_back   = $tb . "\t" . '<a href="javascript:history.back();" id="submit_back" class="button bc-ghost' . $this->add_space( $class_back ) . '" data-ajax="false" data-role="button" data-theme="' . $data_theme_back . '">入力画面に戻る</a>' . "\n";
		$tag_submit = $tb . "\t" . '<input type="submit" id="submit_thanks" value="上記の内容で送信" class="submit_send button' . $this->add_space( $class_send ) . '" data-theme="' . $data_theme_send . '">' . "\n";

		if( $this->is_form() ) {
			$tag .= $tb . "\t" . '<input type="hidden" name="step" value="step_confirm">' . "\n";
			$tag .= $tb . "\t" . '<input type="submit" id="submit_confirm" value="入力内容の確認画面へ" class="submit_send button' . $this->add_space( $class_send ) . '" data-theme="' . $data_theme_send . '">' . "\n";
...
//}
		...
		if( $this->step == 'step_form' ) {
			...
			for( $i = 0; $i < count( $arr ); $i++ ) {
				...
				$tag .= $tb . "\t" . '<span class="input_wrap"><input type="radio" id="' . $id . $this->keta2( $i + $preset_elements_num + 1 ) . '" name="form_' . $id . '" value="' . $val . '" class="input_check' . $res_handle_name . '"' . $checked . '><label for="' . $id . $this->keta2( $i + 1 ) . '" class="check_label">' . $arr[ $i ] . '</label></span>' . "\n";
			}
			if( $handle_name != '' ) {
	...
	private function res_input_checkbox( $type, $id, $elements, $checked_num, $elements_value ) {
		...
		if( $this->step == 'step_form' ) {
			...
			for( $i = 0; $i < count( $arr ); $i++ ) {
				...
				$tag .= $tb . "\t" . '<span class="input_wrap"><input type="checkbox" id="' . $id . $this->keta2( $i + 1 ) . '" name="form_' . $id . '[]" value="' . $val . '" class="input_check"' . $checked . '><label for="' . $id . $this->keta2( $i + 1 ) . '" class="check_label">' . $arr[ $i ] . '</label></span>' . "\n";
			}
			$tag .= $tb;
		} elseif( $this->step == 'step_confirm' ) {
