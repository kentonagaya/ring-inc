...
//$this->debug_repotr .=  '$_SESSION'.'：'.nl2br(var_export($_SESSION,true)).'<br>';
	...
	public function submit ( $class_send = 'bc_blue', $class_back = 'bc_white', $data_theme_send = 'b', $data_theme_back = 'a' ) {
		...
		$tag_back   = $tb . "\t" . '<a href="javascript:history.back();" id="submit_back" class="button bc-ghost' . $this->add_space( $class_back ) . '" data-ajax="false" data-role="button" data-theme="' . $data_theme_back . '">返回输入屏幕</a>' . "\n";
		$tag_submit = $tb . "\t" . '<input type="submit" id="submit_thanks" value="发送上述内容" class="submit_send button' . $this->add_space( $class_send ) . '" data-theme="' . $data_theme_send . '">' . "\n";

		if( $this->is_form() ) {
			$tag .= $tb . "\t" . '<input type="hidden" name="step" value="step_confirm">' . "\n";
			$tag .= $tb . "\t" . '<input type="submit" id="submit_confirm" value="确认输入内容" class="submit_send button' . $this->add_space( $class_send ) . '" data-theme="' . $data_theme_send . '">' . "\n";
	...
	public function must($id, $str = '<span class="tx-icon must-input">必填项</span>' ) {
